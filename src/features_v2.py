"""Stage 04 step 2: *extra* phase-anonymous pair features (v2).

Produces only the NEW columns, keyed by (DeviceId, Detector, cand_phase, win), so they can
be merged onto stage 01's `pair_features_windows.parquet`.  Nothing in stage 01 is overwritten.

New families
------------
1. **Partner geometry** — for every candidate p, the candidate q whose green overlaps p's most
   (found generically, no phase numbers): co-green share, exclusive-green seconds each way,
   green-start / green-end lead of p over q.  Plus `__pdiff` columns = feature(p) - feature(q)
   for the features that discriminate concurrent pairs.
...
3. **Fine ON timing** — sub-second bins of time-since-begin-green, per-cycle first-ON latency
   (median / share <= 1 s / <= 3 s), share of the phase's cycles with any actuation,
   queue-release lag quantiles (which green start ends a long occupancy).
4. **Conditioned call linkage** — 43 within fine bins of an ON, restricted to ONs where the
   candidate is RED (a green phase cannot register a vehicle call) and to ONs where some
   other phase is green; chance corrected.
5. **Shrinkage** — Bayesian-smoothed versions of the key lifts for low-volume detectors.
    ...
    python src/features_v2.py --stage cogreen            # phase-pair co-green table only
"""
...
from features import WINDOW_SETS, apply_window, load_chunk, log  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--out", default="pair_features_v2_extra.parquet")
    ...
    log(f"v2 extras: {len(devs)} signals x {len(wins)} windows")
    parts, t0 = [], time.time()
    ...
    log(f"wrote {out}: {len(df):,} rows x {df.shape[1]} cols, {time.time()-t0:.0f}s")
