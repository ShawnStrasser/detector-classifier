...
sys.path.insert(0, str(HERE / "statewide"))
...
# sibling-relative features (stage 04 list + the v3 yellow/red-clearance family)
...
def add_lag_features(top: pd.DataFrame) -> pd.DataFrame:
    ...
    Sign convention: `lag_peak > 0` means `other` fires AFTER this detector, i.e. this
    detector is the upstream one (Advance).  `coinc_05_excess` near its maximum with
    `lag_peak ~ 0` means the two channels see the *same* vehicles at the same place."""
    lag = _cat(LAG_FILES)
