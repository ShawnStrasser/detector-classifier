"""Extra *function* features: yellow / red-clearance behaviour and pairwise actuation lag.

Both are phase-anonymous (channel numbers and phase numbers are join keys only).

1. **Yellow / red clearance** -- one row per (DeviceId, Detector, cand_phase, win).
...
2. **Detector-pair lag** -- one row per (DeviceId, win, Detector, other): the signed lag from
...
from features import log  # noqa: F401  (re-exported for the research build driver)
