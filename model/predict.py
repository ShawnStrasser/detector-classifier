...
This is the whole product.  Everything it needs is in this folder; see `../README.md`
for install and usage, and `../REPORT.md` for how the model was built.

Command line
------------
    python model/predict.py --events events.parquet --out preds.csv
    python model/predict.py --events day.parquet --device-ids <guid>,<guid> \
...
Models are loaded from `model/weights/` by default:
    phase_lgbm_v4_s{0,1,2}.txt   pair ranker, 3 seeds, averaged
    gru.onnx                     the neural pair scorer (onnxruntime, `gru_onnx.py`)
    ...
    model_card.json              what was trained on what, and the exact feature lists

Two models, one answer.  On samples up to `cutoff_minutes` of data the ranker's
per-detector probabilities are averaged 50/50 with a small recurrent network that reads
the raw second-by-second trace, and the joint decoder then runs on the mixture.  The two
make genuinely different mistakes on short samples, so averaging them removes about a
third of the phase errors at 30 minutes.  Above the cut-off the network is not run at
all: it would cost far more than the fraction of a point it still adds.
The network is evaluated with **onnxruntime on the CPU** -- one runtime, no PyTorch and
no fallback -- and the function model reads the tree pipeline's phase only.
...
*higher* accuracy, because a short sample can still be conclusive -- see ../README.md.
...
import features_partner as f2  # noqa: E402
...
# folder-relative -- no absolute user paths anywhere in the inference path
...
def build_features(con, w0: float, w1: float):
    """Pair features, the partner extras, the yellow / red-clearance and detector-lag
    tables, and the cross-detector similarity graph."""
    global _LAG
    _LAG = None
    secs = max(w1 - w0, 1.0)
    ...
    sim = sim_mod.build_window(con, WIN, secs)
    ...
    # ---- yellow / red-clearance pair features + detector-pair actuation lags ----
...
def score(df: pd.DataFrame, sim: pd.DataFrame, model_dir: Path,
    ...
    probability -- exactly the averaging the training code measured.
    ...
    if gru is not None and len(gru):
        # the function model keeps reading the trees-only phase
...
def _function_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    plus shape, sibling-relative and cross-detector lag aggregates.  Identical code path
    to training; no label and no phase number is used."""
    ...
    if _LAG is not None and len(_LAG):
        top = fn.add_lag_features(top, _LAG)
...
def score_function(df: pd.DataFrame, model_dir: Path) -> pd.DataFrame:
    ...
    # An earlier version forced ambiguous Advance/Presence detectors to Other.  With an
    # explicitly trained Other class it costs accuracy, so it ships disabled; the
