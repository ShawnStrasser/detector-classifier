"""The pure-numpy tree evaluator must reproduce LightGBM, and predict() must run with lightgbm and scipy
absent. Run: python tests/test_numpy_backend.py"""
import os, subprocess, sys
import numpy as np, pandas as pd

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, os.path.join(ROOT, "src"))
MODELS = os.path.join(ROOT, "models", "beta_v0")
...
def test_boosters_match_lightgbm():
    ...
    for name in ("phase_lgbm_v2", "decode_lgbm_v2", "function_lgbm_v2"):
        path = os.path.join(MODELS, name + ".txt")
        ref, mine = lgb.Booster(model_file=path), NumpyBooster(path)
        X = rng.normal(0, 3, (4000, mine.n_features))
        ...
        assert np.allclose(ref.predict(X), mine.predict(X), rtol=0, atol=1e-10), name
        assert np.allclose(ref.predict(X, raw_score=True), mine.predict(X, raw_score=True), atol=1e-10), name
...
if __name__ == "__main__":
    for t in (test_boosters_match_lightgbm, test_pipeline_identical_with_numpy_backend,
