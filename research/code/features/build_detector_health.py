...
from health_sql import HEALTH_FLAG_SQL  # noqa: E402  (same rules as health.flag_detectors)

SKIP_PASSES = '--assemble-only' in sys.argv
WORK = DC_WORK.as_posix()
...
con = duckdb.connect()
con.execute(f"SET memory_limit='6GB'; SET threads=6; "
            f"SET temp_directory='{WORK}/tmp'; SET preserve_insertion_order=false;")
