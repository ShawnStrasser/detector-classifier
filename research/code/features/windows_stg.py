...
Importing this module registers the sets in `build_features.WINDOW_SETS`, so the
feature builders can be driven with `--windows stgmixed` without any edit.
"""
...
bf.WINDOW_SETS["stgmixed"] = WINDOWS_STG_MIXED
bf.WINDOW_SETS["stgshortb"] = WINDOWS_STG_SHORT_B
bf.WINDOW_SETS["stgall"] = WINDOWS_STG_MIXED + WINDOWS_STG_SHORT_B
bf.DURATION_OF["m5"] = 5.0 / 60.0
bf.DURATION_OF["m10"] = 10.0 / 60.0
bf.DURATION_OF[FULL] = 66.0

_orig_win_hours = bf.win_hours


def win_hours(win: str) -> float:
    if win in bf.DURATION_OF:
        return bf.DURATION_OF[win]
    return bf.DURATION_OF[win.split("_")[0]]


bf.win_hours = win_hours
